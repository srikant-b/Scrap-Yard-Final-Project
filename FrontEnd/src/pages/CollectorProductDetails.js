...
const Product = (props) => {
    ...
    const [userCartData, setuserCartData] = useState(initialValue);
    ...
    const handleClickDeny = async (prod_id) => {
        setLoading(true);
        try {
            // await myAxios.delete(`apiCart/deleteScrapMaterialFormCart/${prod_id}`, { headers })

            //ColCustDetailsPage (c_name, c_add, c_ph_no)-->ColScrapMaterialPage (products with accept reject button)
            //c_id scrapMaterial_id present in cart table
            await myAxios.delete(`apiCartColProdPage/deleteScrapMaterialFromColProdPage/${prod_id}`, { headers })
            let res = (await myAxios.get(`apiCart/getAllScrapMaterial/${Token.userDto.id}`, { headers }))
            console.log(res.data);
            dispatch({
                type: "ACCEPT_MATERIAL",
                payload: res.data,
            })
        } catch (error) {
            console.error(error.message);
    ...
    const handleClickAccept = async (cust_id, scrap_id) => {
                                ...
                                <div className="form-group d-grid mt-2">
                                    {
                                        ...
                                        state.cart.some(p => p.id === data.id) ?
                                            (<Button onClick={() => handleClickDeny(data.id)} type='submit' className="btn btn-dark">Deny the Item</Button>)
                                            :
                                            (<Button onClick={() => handleClickAccept(Token.userDto.id, data.id,)} type='submit' className="btn btn-secondary">Accept the Item</Button>)
...
export default CollectorProductDetails